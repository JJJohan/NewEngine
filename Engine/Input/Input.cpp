...
namespace Engine
{
	...
	void Input::CallKeyEvents(int keyCode, bool keyPressed)
	{
		...
		if (it != _keyEvents.end())
		{
			...
			{
				if ((events[i].KeyState == KeyUp && !_keyPressed)
					|| (events[i].KeyState == KeyDown && !_keyPressed))
				{
					events[i].Event();
				}
			}
	...
	void Input::CallMouseEvents(int mouseButton, bool buttonPressed)
	{
		...
		if (it != _mouseEvents.end())
		{
			...
			for (size_t i = 0; i < events.size(); ++i)
			{
				if ((events[i].MouseState == ButtonUp && _mousePressed)
					|| (events[i].MouseState == ButtonDown && !_mousePressed))
				{
					events[i].Event();
				}
			}
		...
		for (auto it = _activeMouseButtons.begin(); it != _activeMouseButtons.end(); ++it)
		{
			...
			if (it->second)
			{
				if (_mouseEvents.find(it->first) != _mouseEvents.end())
				{
					...
					{
						if (eventIter->MouseState == ButtonHeld)
						{
							eventIter->Event();
						}
					}
